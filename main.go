...
func (t Training) distance() float64 {
	...
	tResult := float64(t.Action) * t.LenStep / MInKm
	return tResult
}
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	rResult := (CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours
	return rResult
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	//wInMsec средняя скорость ходьбы в м/с
	wInMsec := w.meanSpeed() * KmHInMsec
	//wInMsec**2
	wInMsec2 := math.Pow(wInMsec, 2)
	//hInCm рост в см
	hInCm := w.Height / CmInM
	wResult := (CaloriesWeightMultiplier*w.Weight + (wInMsec2/hInCm)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
	return wResult
}
...
func (s Swimming) meanSpeed() float64 {
	...
	sMeanSpeed := float64(s.LengthPool) * float64(s.CountPool) / MInKm / s.Duration.Hours()
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	sResult := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
	return sResult
}
